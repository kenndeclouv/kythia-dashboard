...
const NavSearch = ({
	...
	const { lang: locale, id: guildId } = useParams();
	const { isBreakpointReached } = useVerticalNav();

	const data = getSearchData(guilds || [], guildId as string | null, userId);
	...
	const onSearchItemSelect = (item: Item) => {
		item.url.startsWith('http')
			...
			: router.push(
					item.excludeLang
						? item.url
						: getLocalizedUrl(item.url, locale as Locale),
				);
		setOpen(false);
	};
		...
		<>
			{isBreakpointReached ? (
				...
				<div className="flex items-center gap-2">
					...
					<button
						type="button"
						className="whitespace-nowrap select-none text-textDisabled cursor-pointer bg-transparent border-0 p-0"
						onClick={() => setOpen(true)}
					>
						Search ⌘K
					</button>
